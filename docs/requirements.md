...
- **対象市場: 日本市場のみ（日本語コンテンツ）**
- MCPサーバー本体（stdio / 必要に応じてHTTP）
- ビジネスフレームワーク生成ツール群（全部載せ、§6.2）
- 情報ソースアダプター群（後述 §5）
- トレンド集計・差別化分析ツール
- ローカル実行での動作
...
> **対象市場**: 日本国内のみ。Kindle は `amazon.co.jp`、Kobo は楽天Kobo日本版を対象とする。

| # | ソース | 取得手段 | 優先度 | 備考 |
|---|---|---|---|---|
| S-01 | 小説家になろう | 公式「なろう小説API」 | **最高** | Web投稿サイトの主ソース。JSON/YAML、規約準拠で安定 |
| S-02 | Amazon Kindle（日本） | PA-API or 公開ページ | **最高** | **電子書籍の主ソース**。`amazon.co.jp` のみ |
| S-03 | 楽天Kobo（日本） | 楽天ブックスAPI等 | 高 | 電子書籍 第2優先 |
| S-04 | BookLive | Web（公開ページ） | 高 | 電子書籍 第2優先。規約確認必須 |
| S-05 | カクヨム | Web（構造化データ活用） | 中 | 公式API無し。規約確認必須 |
| S-06 | アルファポリス | Web | 中 | 規約確認必須 |
| S-07 | 出版社新刊情報（MF文庫J・電撃・GA等）| RSS/公開ページ | 中 | レーベル動向把握 |
| S-08 | Google Trends（日本） | pytrends（geo=JP） | 中 | 題材の検索需要を補完 |
| S-09 | BookWalker | Web | 低 | 電子書籍 第3優先 |
| S-10 | ノベルアップ+ | Web | 低 | 規模小・規約確認必須 |
| S-11 | X（旧Twitter）ハッシュタグ | X API | 低 | 有料APIコスト要検討 |

**実装順（推奨）**: S-01（なろう）→ S-02（Kindle）→ S-03（Kobo）＋ S-04（BookLive）→ 以降は必要に応じて追加
...
│       ├── cache.py
│       ├── adapters/            # 情報ソース毎（§5.1 の優先度順）
│       │   ├── base.py
│       │   ├── narou.py         # S-01 最高
│       │   ├── kindle_jp.py     # S-02 最高（amazon.co.jp）
│       │   ├── kobo_jp.py       # S-03 高
│       │   ├── booklive.py      # S-04 高
│       │   ├── kakuyomu.py      # S-05 中
│       │   ├── alphapolis.py    # S-06 中
│       │   ├── publishers.py    # S-07 中
│       │   ├── google_trends.py # S-08 中（geo=JP）
│       │   ├── bookwalker.py    # S-09 低
│       │   ├── novelup.py       # S-10 低
│       │   └── twitter.py       # S-11 低
...
## 10. MVPスコープ（v0.1.0 リリース目標）

> **基本方針**: 最小構成で素早く動かす。残りは段階的に追加（後述 §11）。

### 10.1 MVPに含めるもの

| 区分 | 内容 |
|---|---|
| 情報ソース | S-01 **小説家になろう（なろう小説API）** / S-02 **Amazon Kindle（日本）** |
| フレームワーク | **3C / STP / 4P / SWOT / ペルソナ** の5つ |
| 取得・分析ツール | `search_works`, `get_ranking`, `analyze_trends`, `list_frameworks` |
| 配布 | PyPI、`uvx novel-searah-mcp` で起動可能 |
| 設定 | 最小限の環境変数（APIキー、キャッシュディレクトリ等） |

### 10.2 MVPに含めないもの（段階追加）

- S-03以降のアダプター（Kobo, BookLive, カクヨム, etc.）
- §6.2 の残り21フレームワーク
- `compare_works`, `evaluate_title`, `suggest_kpi` 等の高度ツール
- SSE/HTTP トランスポート（stdio のみ）
...
- [x] 電子書籍ソースの優先順位（Kindle > Kobo / BookLive、日本市場のみ）
- [x] MVPスコープの絞り込み（§10 参照）
- [ ] アダプター・フレームワーク基底クラスのインターフェース詳細設計 → **`docs/design.md` に切り出し**
- [ ] キャッシュ戦略（TTLデフォルト、キー設計） → `docs/design.md`
- [ ] 設定ファイル形式（環境変数 / TOML / JSON） → `docs/design.md`
- [ ] なろう小説API・PA-APIの利用規約／認証方法の精査
...
| 2026-04-23 | v0.3 | 対象市場を日本のみに確定。情報ソース優先度を再定義（§5.1）。MVPスコープ（§10）を明記。基底クラス設計は `docs/design.md` へ分離 |
